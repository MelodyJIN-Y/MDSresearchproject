...
keep.genes <- numzero.genes < (ncol(cm)-15)
...
# treat.all$p.value
perm.size<- 1000
#modt.perm.array<- readRDS("modt_perm_array_indall.Rds")
#t.perm.array<- readRDS("t_perm_array_indall.Rds")
...
toc(log=TRUE)
#saveRDS(modt.perm.array, "modt_perm_array_indall.Rds")
#saveRDS(t.perm.array, "t_perm_array_indall.Rds")
##########
# modt-stat
modt.perm.pvals<-apply(expand.grid(x = 1:dim(logcounts.all)[1], 
                           y = 1:dim(design)[2]), 1, 
                function(r) (sum(modt.perm.array[r[1],r[2], ]> obs.modtstat[r[1],r[2]])+1)/(perm.size+1) )
...
pdf("hist_all.pdf")
hist(obs.t.pval,main = "p-values by limma", xlab  ="p-values (t-stat)" )
hist(obs.t.pval.adj,main = "adjusted p-values by limma", xlab  ="adjusted p-values (t-stat)" )
hist(t.perm.pval,main = "p-values by permutation", xlab  ="p-values (t-stat)" )
hist(t.perm.pval.adj,main = "Adjusted p-values by permutation", xlab  ="adjusted p-values (t-stat)" )

hist(obs.mod.pval,main = "p-values by limma", xlab  ="p-values (modt-stat)" )
hist(obs.mod.pval.adj,main = "adjusted p-values by limma", xlab  ="adjusted p-values (modt-stat)" )
hist(modt.perm.pval,main = "p-values by permutation", xlab  ="p-values (modt-stat)" )
hist(modt.perm.pval.adj,main = "Adjusted p-values by permutation", xlab  ="adjusted p-values (modt-stat)" )
dev.off()
library(ggvenn)
for (i in 1:8){
...
save.all.sig.vis(8, "tstat_indall_p", "tstat_sig_indall_p",t.perm.pval, obs.t.pval,"pvalue" )
...
for (i in 1:8){
  ...
  up_reg_genes_limma <- row.names(obs.mod.pval[obs.t.pval[,i]<0.05,])
  up_reg_genes_perm<- row.names(modt.perm.pval[t.perm.pval[,i]<0.05,])
...
for (i in 1:8){
  print("########  (one-sided) moderated-t")
  up_reg_genes_limma <- row.names(obs.mod.pval[obs.mod.pval[,i]<0.05,])
  up_reg_genes_perm<- row.names(modt.perm.pval[modt.perm.pval[,i]<0.05,])
  # print(paste("# false positive genes",sum(perm.pval[up_genes, i] >= 0.05)))
  #print(paste("# of up-regulated genes for ", colnames(modt.perm.pval.adj)[i], "cells"))
  print(paste("number of up-regulated genes by limma", length(up_reg_genes_limma)))
  print(paste("number of up-regulated genes by permutation", length(up_reg_genes_perm)))
  print(paste("shared in common:",length(intersect(up_reg_genes_limma, up_reg_genes_perm))))
