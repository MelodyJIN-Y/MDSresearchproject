...
saveRDS(adj.p, "V2_50r_adj.p.Rds")
```
...
adj.p$cp<- rep(cp.ft,times=50,each = 4*5)
...
plot.result(df=adj.p, name="all.res",  clrs = c('lightsteelblue2','navajowhite2',
                 "steelblue2","darkgoldenrod1",
                 "royalblue1","coral","blue4","tomato3",
                 "green4"))
plot.result(df=adj.p.tt, name="treatt.res", clrs = c("steelblue2","darkgoldenrod1"))
plot.result(df=adj.p.rest, name="t_stat.res", clrs = c('lightsteelblue2','navajowhite2',
                 "royalblue1","coral","blue4","tomato3",
                 "green4"))

```
lets vary DE proportion
```{r}
groups_lst<- list(c(0.01, 0.99),
                  c(0.05, 0.95),
                  c(0.1, 0.9),
...
for (rep in 1: rep.times){
  ...
  for (cp in 1:length(groups_lst)){
    sim <- splatSimulate(curr_params, method = "groups", nGenes = 5000,
                         batchCells = 1000, dropout.type = "none", out.prob = 0, 
                         de.prob = c(0.2, 0.02),de.downProb = c(0, 0),
                         group.prob = groups_lst[[cp]],
                         #seed = seed.number.cp[cp], 
                         seed = rep.seed[cp], 
                         de.facLoc = c(1.1, 1.1),
