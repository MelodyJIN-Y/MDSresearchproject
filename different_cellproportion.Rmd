...
for (rep in 1: rep.times){
  ...
  for (cp in 1:length(groups_lst)){
    ...
    setwd("/Volumes/Expansion/Data Science Research Project/MDSresearchproject/50run_7cp/20run_3cp_g9897")
    # set the identifier for the plots 
    prefix<-paste("s",rep.seed[cp], paste(as.character(table(sim$Group)),collapse="_"), sep="_")
    prefix<-paste("/Volumes/Expansion/Data Science Research Project/MDSresearchproject/50run_7cp/20run_3cp_g9897/", 
                  prefix, sep="")
    ############################################################################
    # run permutation for all stats
    n.cores=parallel::detectCores()-8
...
toc(log = T)
saveRDS(adj.p, "r30.3ct.adj.p.Rds")
```
...
adj.p$cp<- rep(paste("group1:group2:group3 = ",cp.ft, sep=""),times=20,each = 6*5)
...
adj.p$cp<- rep(cp.ft,times=5,each = 4*5)
# combined = TRUE
...
adj.p.rest<- adj.p[adj.p$category %in% ntstat_lst, ]

adj.p.less<-adj.p[adj.p$cp %in% c("5_95","10_90","20_80","30_70","40_60","50_50"), ]


plot.result<-function(df, name, clrs){
  ...
  ggsave(paste(name, "pdf",sep="."), width = 23, height=12)
  
}

plot.result(df=adj.p, name="adj.p.r20.3ct.all",  clrs = c('lightsteelblue2','navajowhite2',
                 "steelblue2","darkgoldenrod1",
                 "royalblue1","coral","blue4","tomato3",
                 "green4"))
plot.result(df=adj.p.tt, name="adj.p.r20.3ct.tt", clrs = c("steelblue2","darkgoldenrod1"))
plot.result(df=adj.p.rest, name="adj.p.r20.3ct.rest", clrs = c('lightsteelblue2','navajowhite2',
                 "royalblue1","coral","blue4","tomato3",
                 "green4"))
